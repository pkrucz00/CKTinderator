...
class DifferentiableLandmarks(torch.nn.Module):
    def __init__(self, generator, landmarks_model, images: torch.Tensor = None, random_vectors_num=0):
        super().__init__()
        self.z = torch.nn.Parameter(pick_best_random_z(images, generator, n=random_vectors_num))
...
def find_z(images: torch.Tensor, lr=1e-2, iters=1000, alpha=0, beta=1, init_vectors_num=0, save_every=None) -> torch.Tensor:   
    generator = load_generator().to("cuda:0")
    landmarks_model = load_landmarks_model()
    model = DifferentiableLandmarks(generator, landmarks_model, images, random_vectors_num=init_vectors_num)
    ...
    for i in range(iters):
        y_pred = model()
        loss = alpha * criterion(images, model.curr_image) + beta + criterion(y_target, y_pred) 
